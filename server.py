...
# ─── aiohttp WebSocket handler (Render — same port HTTP upgrade) ─────────────
async def handle_ws(request):
    """WebSocket via HTTP upgrade on /ws — ใช้บน Render ที่มี port เดียว"""
    ws = web.WebSocketResponse(heartbeat=30)
    await ws.prepare(request)

    connected_clients.add(ws)
    log.info(f"WS client connected via /ws ({len(connected_clients)})")
                ...
                try:
                    ...
                    if cmd == "start":
                        ...
                        for c in connected_clients:
                            try:
                                await c.send_str(json.dumps({"type": "status", "running": True, "camera_ok": True}))
                            except Exception:
                                pass
                    elif cmd == "stop":
                        state["running"] = False
                        ...
                        for c in connected_clients:
                            try:
                                await c.send_str(json.dumps({"type": "status", "running": False, "camera_ok": state["camera_ok"]}))
                            except Exception:
                                pass
                    elif cmd == "reset":
                        state.update({"counts": {"AA":0,"A":0,"B":0,"C":0}, "history": [], "total": 0})
                        for c in connected_clients:
                            try:
                                await c.send_str(json.dumps({"type": "reset"}))
                            except Exception:
                                pass
                    elif cmd == "set_interval":
                        global CAPTURE_INTERVAL
                        ...
                        for c in connected_clients:
                            try:
                                await c.send_str(json.dumps({"type": "config", "interval": CAPTURE_INTERVAL}))
                            except Exception:
                                pass
                except Exception as e:
...
async def main():
    ...
    app.router.add_get( "/ws",             handle_ws)            # WebSocket via HTTP upgrade (Render)
    ...
    # WebSocket port แยก (local mode เท่านั้น)
    if not IS_RENDER:
        ws_server = await websockets.serve(ws_handler, WS_HOST, WS_PORT)
        log.info(f"WS    → ws://0.0.0.0:{WS_PORT}")
    else:
        log.info(f"WS    → wss://[domain]/ws  (HTTP upgrade)")
